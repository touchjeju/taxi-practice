# 택시 연습하기 - 사이트에 올리기
# menu/ app/ common/ teacher/ join/ 의 내용을 docs/ 로 복사한 뒤 GitHub 에 올린다.
...
New-Item -ItemType Directory -Force -Path `
    'docs\img', 'docs\app\img', 'docs\common', 'docs\teacher', 'docs\join' | Out-Null
Copy-Item 'menu\index.html' 'docs\index.html' -Force
Copy-Item 'menu\img\*' 'docs\img\' -Force -Recurse
Copy-Item 'app\index.html', 'app\style.css', 'app\app.js', 'app\track.js' 'docs\app\' -Force
Copy-Item 'app\img\*' 'docs\app\img\' -Force -Recurse
Copy-Item 'common\*.js' 'docs\common\' -Force
Copy-Item 'teacher\index.html' 'docs\teacher\' -Force
Copy-Item 'join\index.html' 'docs\join\' -Force
...
function Stamp($p) {
    ...
    $t = $t -replace 'src="track\.js(\?v=\d+)?"',         ('src="track.js?v=' + $ver + '"')
    $t = $t -replace '"\.\./common/fb\.js(\?v=\d+)?"',    ('"../common/fb.js?v=' + $ver + '"')
    $t = $t -replace '"\.\./common/steps\.js(\?v=\d+)?"', ('"../common/steps.js?v=' + $ver + '"')
    [IO.File]::WriteAllText($p, $t, (New-Object Text.UTF8Encoding $false))
}
Stamp 'docs\app\index.html'
Stamp 'docs\app\track.js'
Stamp 'docs\join\index.html'
Stamp 'docs\teacher\index.html'
Write-Host ('[1/3] 메뉴 · 앱 · 교사용 · 학생용을 docs 폴더로 복사했습니다. (버전 ' + $ver + ')') -ForegroundColor Green
...
# 2. 바뀐 게 있는지 확인
git add docs firestore.rules firebase.json | Out-Null
$changed = git status --porcelain docs firestore.rules firebase.json
if (-not $changed) {
    Write-Host ''
...
Write-Host '  https://touchjeju.github.io/taxi-practice/          (수강생 자유 연습)' -ForegroundColor White
Write-Host '  https://touchjeju.github.io/taxi-practice/teacher/  (교사용)' -ForegroundColor White
Write-Host ''
Write-Host '접근 규칙(firestore.rules)을 고쳤다면 아래도 한 번 실행해 주세요.' -ForegroundColor DarkGray
Write-Host '  firebase deploy --only firestore:rules --project ttest-af6cf' -ForegroundColor DarkGray
Write-Host ''
